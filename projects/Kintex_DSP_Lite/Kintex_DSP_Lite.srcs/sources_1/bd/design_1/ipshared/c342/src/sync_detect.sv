...
assign stat_num     = 8'd25;//(sync_mode)? 8'd25:8'd50;	
assign vrf_time_sop = time_sop<<1;//(sync_mode)? time_sop<<1 : {1'b0,time_sop};
  //------------------------------------------------------------------------------------------------------
  //--TRESHOLD_DETECT
  //------------------------------------------------------------------------------------------------------
  ...
  //------------------------------------------------------------------------------------------------------
		...
		always_ff @ (posedge iclk or negedge ireset) begin
			if	(~ireset)
				watchdog_cnt <= '0;
				else if	(~trh_auto_rg[0] || osop_vrf) watchdog_cnt <= '0;
   ...
   // #(
	...
	always_ff @ (posedge iclk or negedge ireset) begin
			if	(~ireset)
                ...
                begin				
			    if (st_oval_0 && (((ostat_0 > (stat_num - pSTAT_Gap)) && (ostat_0 < (stat_num+pSTAT_Gap))))) vrf_hold <= 8'd1; //~sync_mode_stb
                    else if (st_oval_0) vrf_hold <= 8'd0;//|| sync_mode_stb	    
				end
			    else vrf_hold <= 8'd0;
	...
	always_ff @ (posedge iclk or negedge ireset) begin
			if	(~ireset)
				...
				begin
				ostat_data <= {3'd0,vrf_hold,3'd0,trh_hold,8'd0,ostat_0,ostat_1};              		
				end			
		end	
