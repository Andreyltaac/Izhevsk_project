...
assign osop_vrf     = osop_trg[1] && val_sop; //&& vrf_even_val; 
   ...
   #(
	...
	always_ff @ (posedge iclk or negedge ireset) begin
			if	(~ireset)
                ...
                begin				
			    if (~sync_mode_stb && st_oval_1 && (((ostat_1 > (stat_num - pSTAT_Gap)) && (ostat_1 < (stat_num+pSTAT_Gap))))) vrf_hold <= 8'd1;
	...
	always_ff @ (posedge iclk or negedge ireset) begin
			if	(~ireset)
				...
				begin
				ostat_data <= {3'd0,vrf_hold,3'd0,trh_hold,8'd0,ostat_1,ostat_0};              		
				end			
		end	
