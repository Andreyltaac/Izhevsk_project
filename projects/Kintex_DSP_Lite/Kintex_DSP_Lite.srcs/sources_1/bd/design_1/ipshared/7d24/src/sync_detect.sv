...
assign stat_num_trh = (mode_pss_one)? stat_num - 8'd25:stat_num; 
assign vrf_hold     = vrf_hold_full || vrf_hold_cut;
assign ostat_vrf    = ostat_1 + ostat_2; 
assign osop_vrf_0   = osop && val_sop && vrf_even_val; 
	...
	always_ff @ (posedge iclk or negedge ireset) begin
			if	(~ireset)
			    ...
			    if (trh_auto_rg[0] && ~trh_auto_rg[1]) trh_lvl_upd <= itrh_lvl; 
			         ...
			         else if (st_oval_0 && (ostat_0 < (stat_num_trh - pSTAT_Gap)) && trh_lvl_upd > 12'd50) 
					 begin
					 if (ostat_0 < (stat_num_trh-8'd20)) trh_lvl_upd <= trh_lvl_upd - pSTAT_step_high; 
					   else 
					   begin 
			         ...
			         else if (st_oval_0 && (ostat_0 > (stat_num_trh + pSTAT_Gap))) 
				         begin
						 if (ostat_0 > (stat_num_trh + 8'd55)) trh_lvl_upd <= trh_lvl_upd + pSTAT_step_high; 
						 else if (ostat_0 > (stat_num_trh + 8'd25)) trh_lvl_upd <= trh_lvl_upd + pSTAT_step_mid;
						      else 
							  begin 
		 ...
		 always_ff @ (posedge iclk or negedge ireset) begin
			if	(~ireset)
				val_sop <= 1'b0;
			       ...
			       else if (cnt_time_sop == vrf_time_sop - pVERF_Gap)  
				        begin 
						val_sop <= 1'b1;
						vrf_even_val <= ~vrf_even_val;
						end
   ...
   #(
	...
	always_ff @ (posedge iclk or negedge ireset) begin
			if	(~ireset)
                ...
                begin				
			    if (~sync_mode_stb && st_oval_1 && (((ostat_vrf > (stat_num - pSTAT_Gap)) && (ostat_vrf < (stat_num+pSTAT_Gap))))) vrf_hold_full <= 8'd1; 
                    else if (st_oval_1 || sync_mode_stb) vrf_hold_full <= 8'd0;	    
				end
			    else vrf_hold_full <= 8'd0;
	...
	always_ff @ (posedge iclk or negedge ireset) begin
			if	(~ireset)
                ...
                begin				
			    if (~sync_mode_stb && st_oval_1 && ((~sync_mode &&((ostat_vrf > ((stat_num>>1)- pSTAT_Gap)) && (ostat_vrf < ((stat_num >> 1)+pSTAT_Gap)))))) vrf_hold_cut <= 8'd1; 
                    else if (st_oval_1 || sync_mode_stb) vrf_hold_cut <= 8'd0;	    
				end
			    else vrf_hold_cut <= 8'd0;
	...
	always_ff @ (posedge iclk or negedge ireset) begin
			if	(~ireset)
				...
				begin
				ostat_data <= {2'd0,mode_pss_one,vrf_hold,3'd0,trh_hold,ostat_2,ostat_1,ostat_0};              		
				end			
		end	
	...
	always_ff @ (posedge iclk or negedge ireset) begin
			if	(~ireset)
				...
				begin
				if (~sync_mode && vrf_hold_cut && st_oval_2)
				mode_pss_one <= 1'd1;
                else 
				mode_pss_one <= 1'd0;
				end			
