   ...
   #(
	...
	always_ff @ (posedge iclk or negedge ireset) begin
			if	(~ireset)
                ...
                begin				
			    if (~sync_mode_stb && st_oval_1 && (((ostat_vrf > (stat_num-pSTAT_Gap)) && (ostat_vrf < (stat_num+pSTAT_Gap)))|| (~sync_mode && trh_hold &&((ostat_vrf > ((stat_num>>1)-pSTAT_Gap)) && (ostat_vrf < ((stat_num >> 1)+pSTAT_Gap)))))) vrf_hold <= 8'd1; 
	...
	always_ff @ (posedge iclk or negedge ireset) begin
			if	(~ireset)
				...
				begin
				ostat_data <= {3'd0,vrf_hold,3'd0,trh_hold,ostat_2,ostat_1,ostat_0};              		
				end			
		end	
